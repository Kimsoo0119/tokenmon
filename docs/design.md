...
// config.json 핵심 스키마
{
  ...
  "evolutionBlock": null,        // B 키로 막아둔 진화 { monster, idx, blockedTo }
...
계산된 단계를 화면에 바로 반영하지 않고, 메인 프로세스가 **표시 단계**(`shownIdx`)를 따로 들고 폴링 결과와 비교합니다. 단계가 올라도 자동으로 진화하지 않습니다 — 사용자가 놓치지 않도록 **대기 상태**로 멈춰 알리고, **사용자의 선택으로** 진행합니다 (`evolutionCinematic: true`, 설정에서 끄면 기존처럼 짧은 플래시로 즉시 반영):

1. **대기** — 말풍선 "어…? ㅇㅇ의 상태가…!", 펫에 금빛 글로우, 트레이 `✨Lv.N`(사용량·오류 표시를 가리지 않게 병기), macOS 알림. 사용자가 펫이나 알림을 클릭할 때까지 이전 단계로 기다립니다. 대기 말풍선은 자동으로 사라지지 않고 클릭할 때까지 유지됩니다 — 드래그하면 따라오고, 외부 알림 등 임시 말풍선이 잠깐 덮어도 걷히면 다시 돌아옵니다. 글로우는 스프라이트가 아닌 별도 오버레이의 opacity만 움직입니다(컴포지터 합성 전용 — 대기가 며칠 가도 리페인트 비용이 없고, 공격·점프 애니메이션 슬롯을 차지하지 않습니다). 주간 롤링 윈도로 %가 임계값 근처를 오르내려도 같은 진화의 시스템 알림은 한 번만 보냅니다. 기다리는 동안 더 높은 임계값을 넘으면 목표 단계만 올라가고, 주간 리셋으로 %가 떨어지면 대기가 조용히 풀립니다
2. **연출 (클릭 후 4.5초)** — "ㅇㅇ의 모습이 변하기 시작했다!" 말풍선과 함께 실루엣 점멸
3. **커밋** — 3연속 플래시 절정에서 다음 단계 GIF로 교체, "축하합니다! ㅇㅇ은(는) ㅇㅇ(으)로 진화했습니다!" 말풍선 + 알림
4. **이스터에그 (UI에 노출하지 않음)** — 연출 구간에 **B 키**를 누르면 게임처럼 진화가 멈춥니다. 차단은 `evolutionBlock: { monster, idx, blockedTo }`로 config에 저장되고, **주간 리셋되면 해제**, **차단했던 것보다 높은 임계값을 넘으면 재시도**합니다 (`resolveStage`, 재시도 경로의 차단은 진화가 실제로 커밋될 때 지웁니다 — 클릭 전에 앱이 꺼져도 차단이 살아남게). B는 전역 단축키가 아니라 **펫 창 로컬 keydown**으로 받습니다 — 연출은 펫 클릭 직후라 창이 포커스를 갖고 있고, 전역으로 잡으면 다른 앱에서 치던 b가 몰래 진화를 취소해버립니다. config에서 읽은 차단은 `validBlock`으로 모양을 검증해 손편집·단계 삭제로 어긋난 값을 무시합니다
5. 조사(은/는, 으로/로)는 이름 마지막 글자의 받침으로 고릅니다 (`particle`). ㄹ받침 뒤에는 '로'를 씁니다

연출과 무관하게 즉시 반영하는 경우: 첫 표시(시작 직후·몬스터 교체·소스 전환)와 단계 하강(주간 리셋 — 기존 플래시만). 대기 상태는 저장하지 않으므로 대기 중에 앱을 재시작하면 연출 없이 진화된 모습으로 뜹니다. 연출 중에 설정이 바뀌면 차단 저장 없이 조용히 접습니다. 오케스트레이션(대기·타이머·취소·차단 저장)은 메인이, 시각 효과는 펫 렌더러가 맡습니다 — 상태 푸시의 `pendingEvolution` 플래그(글로우·클릭 해석)와 `evolve-go` / `evolve-start` / `evolve-commit` / `evolve-cancel` IPC로 오갑니다.
...
진화 로직(% → 단계 계산, 리셋 회귀, 임계값 검증, 진화 차단 판정, 한국어 조사 선택), Codex JSONL 파서, config 병합, PokeAPI 슬러그 검증을 순수 함수로 떼어내 `node:test`로 검증합니다. Electron이나 네트워크에 기대지 않아 CI에서 Electron 바이너리 없이 돕니다.
