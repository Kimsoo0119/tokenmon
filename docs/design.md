...
- 펫 · 패널 · 말풍선 창의 생명주기 관리
- 1분 간격 사용량 폴링 후 각 창에 IPC로 전달 (`pollIntervalMin`으로 조정)
...
- **429 응답**: usage API에는 시간당 호출 제한이 있어 폴링 주기가 짧으면 걸립니다. `retry-after` 헤더만큼(없으면 1시간) 조회를 멈추고 여유분 30초를 더해 재시도합니다. 백오프가 걸린 동안에는 호출 없이 마지막 값을 그대로 보여주므로, 폴링 주기를 짧게 잡아도 실제 호출 빈도는 API 쪽 제한을 넘지 않습니다. Codex는 로컬 파일만 읽기 때문에 이 제한과 무관합니다.
