...
1. **대기** — 말풍선 "어…? ㅇㅇ의 상태가…!", 펫에 금빛 글로우, 트레이 `✨`, macOS 알림. 사용자가 펫이나 알림을 클릭할 때까지 이전 단계로 기다립니다. 대기 말풍선은 자동으로 사라지지 않고 클릭할 때까지 유지됩니다 — 드래그하면 따라오고, 외부 알림 등 임시 말풍선이 잠깐 덮어도 걷히면 다시 돌아옵니다. 기다리는 동안 더 높은 임계값을 넘으면 목표 단계만 올라가고, 주간 리셋으로 %가 떨어지면 대기가 조용히 풀립니다
2. **연출 (클릭 후 4.5초)** — "ㅇㅇ의 모습이 변하기 시작했다!" 말풍선과 함께 실루엣 점멸
3. **커밋** — 3연속 플래시 절정에서 다음 단계 GIF로 교체, "축하합니다! ㅇㅇ은(는) ㅇㅇ(으)로 진화했습니다!" 말풍선 + 알림
4. **이스터에그 (UI에 노출하지 않음)** — 연출 구간에 **B 키**를 누르면 게임처럼 진화가 멈춥니다. 차단은 `evolutionBlock: { monster, idx, blockedTo }`로 config에 저장되고, **주간 리셋되면 해제**, **차단했던 것보다 높은 임계값을 넘으면 재시도**합니다 (`resolveStage`). B는 `globalShortcut`으로 연출 구간 4.5초 동안만 시스템 전역 등록합니다 — 항상 잡아두면 다른 앱의 b 입력을 계속 삼키기 때문입니다
