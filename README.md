...
`session`으로 여러 터미널의 세션을 구분해 합산합니다: 하나라도 작업 중이면 볼 유지(2개
이상이면 볼 옆에 ×N 배지), 마지막 세션이 끝날 때만 딸깍+터짐 연출. `waiting`은 일회성
알림으로, 다른 세션이 작업 중이면 볼을 유지한 채 말풍선만 띄웁니다. `project`(프로젝트
폴더명)를 넣으면 말풍선에 어디서 온 이벤트인지 표시됩니다. 볼 상태가 꼬이면 볼을 클릭해
바로 꺼낼 수 있고, 이벤트 없이 30분 지난 세션은 자동 제거됩니다.
